...
class MovieRecommendedPage extends StatefulWidget {
  final String title;
  final List<ChosenMovie> chosenMovies; 
  const MovieRecommendedPage({Key key, this.title = "Movies", this.chosenMovies})
      : super(key: key);

  @override
  _MovieRecommendedPageState createState() => _MovieRecommendedPageState();
}

class _MovieRecommendedPageState extends ModularState<MovieRecommendedPage, MoviesController> {
...
static const MobileAdTargetingInfo targetingInfo = MobileAdTargetingInfo(
 ...
 InterstitialAd createInterstitialAd() {
  ...
  @override
  void initState() {
    //print(widget.chosenMovies[3]);
    int size = widget.chosenMovies.length;
    countRecommend();
    print("size: " + size.toString());
    Random random = new Random();
    randomNumber = random.nextInt(size);
    print("random: " + randomNumber.toString());
    ChosenMovie movie = widget.chosenMovies[randomNumber];
    print("here");
    controller.getVideo(movie.id);
    controller.getReviews(movie.id);
    controller.getWatchProviders(movie.id);
...
//ChosenMovie m = widget.chosenMovies;
  ...
  Widget build(BuildContext context) {
    ...
    if (movie.release_date != null) {
      releaseDate = getDate(movie.release_date);
    }
    if (movie.genre_ids != null){
      genresNames = getGenre(movie.genre_ids);
    }     
    // print("AQUI");
    // controller.getVideo(movie.id);
    // print("DEPOIS VIDEO");
                  ...
                  children: [
                    Container(
                      ...
                      Container(
                          child: Opacity(
                              opacity: 0.8,
                              child: movie.backdrop_path != null ? Image.network(
                      ...
                      Container(
                        ...
                        child: MaterialButton(
                                onPressed: () {
                                  Modular.to.pushReplacementNamed("/movieRecommended", arguments: widget.chosenMovies);
                                },
                    ...
                    Container(
                      padding: EdgeInsets.only(
                          top: scale*20, bottom: scale*10, left: 20, right: 20),
                      child: genresNames.length > 0 ? Text(
                        'string-gender'.i18n() + genresNames,
                        textAlign: TextAlign.center,
                    ...
                    Column(
                      children: [
                        Container(
                          ...
                          height: movie.overview.length < 200 ? scale*120 : scale*180,
                          //color: Colors.white,
                          alignment: Alignment.center,
                          ...
                          child: SingleChildScrollView(
                            child: Container(
                              ...
                              child: AutoSizeText(movie.overview.length > 0 ? movie.overview : 'string-no-overview'.i18n(),
                    ...
                    Observer(builder: (_) {
                      return Row(
                          ...
                          mainAxisAlignment: (controller.video != null &&
                          ...
                          children: [
                            ...
                            Observer(builder: (_) {
                              return Container(
                                  ...
                                  //alignment: Alignment.center,
                                  padding: EdgeInsets.only(top: scale*20),
                                  child: (controller.isReview == true)
                                      ? Row(children: [
                                          GestureDetector(
                                              child: Row(
                                                //mainAxisAlignment: MainAxisAlignment.center,
                    ...
                    Observer(builder: (_) {
                      return Container(
                          padding: EdgeInsets.only(
                              top: scale*15, left: 20, right: 20, bottom: scale*10),
                    ...
                    Observer(builder: (_) {
                      return Container(
                        height: (controller.tapReviews == true &&
                                controller.reviews.length < 1 &&
                                controller.reviews[0].content.length < 200)
                            ? scale*200
                            : scale*height - 300,
                            ...
                            ? ListView.builder(
                                ...
                                itemBuilder: (_, index) {
                                  return ListTile(
                                    ...
                                    title: Text(
                                      controller.reviews[index].author,
                                      style: TextStyle(
                                          color: Colors.white,
                                          fontSize: scale*16,
                                          fontWeight: FontWeight.bold),
