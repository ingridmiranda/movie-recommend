...
abstract class _MoviesControllerBase with Store {
 ...
 @action
  Future getVideo(int movieID) async{
    try {
      Response response = await Dio().get(
        'https://api.themoviedb.org/3/movie/${movieID}/videos?api_key=${Constants.API_DBMOVE_KEY}&language=${language}');
  ...
  @action
  Future getReviews(int movieID) async{
    try {
      Response response = await Dio().get(
        'https://api.themoviedb.org/3/movie/${movieID}/reviews?api_key=${Constants.API_DBMOVE_KEY}&language=${language}&page=1');
      print(response.request);
        print("GET REVIEWS");
        print(response.data['results']);
  ...
  @action 
  Future getWatchProviders(int movieID) async{
    try {
      Response response = await Dio().get('https://api.themoviedb.org/3/movie/${movieID}/watch/providers?api_key=${Constants.API_DBMOVE_KEY}');   
      print(response);
